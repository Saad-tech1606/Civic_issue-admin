  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://backend-civic.onrender.com/user/user_insert",
        {
          fullname,
          phoneNumber,
          email,
      ...
      <div className="relative w-full max-w-lg p-8 rounded-2xl shadow-2xl bg-white/10 backdrop-blur-xl border border-white/20 z-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <input
            ...
            required
            className="px-4 py-3 rounded-xl bg-gray-900/60 text-white border border-gray-700 
            focus:border-blue-500 focus:ring focus:ring-blue-500/30 outline-none text-base"
          />
          ...
          <input
            ...
            required
            className="px-4 py-3 rounded-xl bg-gray-900/60 text-white border border-gray-700 
            focus:border-purple-500 focus:ring focus:ring-purple-500/30 outline-none text-base"
          />
          ...
          <button
            type="submit"
            className="mt-2 py-3 w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white 
            font-bold rounded-xl shadow-lg hover:scale-105 hover:shadow-2xl transition-all text-lg"
          >
            Sign Up
          </button>
        </form>
